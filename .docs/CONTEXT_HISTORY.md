...
- **Expression 카드 흰색 하이라이트·탭 영역**: Profile Saved / Result / History에서 **카드 전체 탭**(북마크 제외)으로 `GET …/expressions/{i}/sound` 재생. 목록·캐러셀마다 **최근 탭 1카드만** 배경 `#FFFFFF` 유지(재생 종료 후에도). fetch 중 16×16 `CircularProgressIndicator`(strokeWidth 2, `#0CABA8` 70%), 재생 중 `megaphone_fill.png` `#0CABA8`, 기본 `megaphone.png` `#0CABA8`. 구현: `lib/screens/profile.dart`, `lib/screens/roleplay/result.dart`, `lib/screens/roleplay/history.dart`. 문서: `.docs/CONTEXT_SCREEN.md` §3·§19.
...
- **Result V2 Expression 카드 메가폰·북마크**: 메가폰 `GET /v1/roleplays/results/{id}/expressions/{i}/sound`(`TtsResultDto`·Playing과 동일 CDN/bytes·`just_audio`), 로딩/재생 중 틴트 `#121212`, 타 카드 탭 시 이전 재생 중단; 재생 완료 후 메가폰만 `#0CABA8`(흰 배경 선택은 유지·상세 위 메모). 북마크 OFF→ON: `POST /v1/users/expressions` `{roleplayResultId, expressionIndex}`, 200 시 `bookmark_on` + l10n `expressionSavedToProfile`. ON→OFF: `DELETE`(history_v2와 동일 쿼리), 200 시 `bookmark_off`만(토스트 없음). 실패 시 HTTP 코드·간단 문구 토스트. V2 진입 시 북마크 UI는 전부 off. 구현·문서: `lib/screens/roleplay/result_v2.dart`, `RoleplayApi.getResultExpressionSound`, `UserApi.saveUserExpression`/`deleteUserExpression`, `.docs/CONTEXT_SCREEN.md` §17-1, `.docs/CONTEXT_ROLEPLAY.md` §6-9.
- **Result V2 본문 타이밍·스타일**: 박스↔Feedback gap 20(스크롤 상단 패딩 반영). effect 완료 후 Feedback 즉시·Expression Upgrade 500ms 후·Got it!/Report 1s 후 동시 노출 + 240ms `easeOut` fade-in. Expression 카드 배경 `#80D7CF`, rephrasedSentence·meaningUserLanguage 행 좌 패딩 +30(세로 순서: rephrased → meaning).
...
- **공지사항 화면**: AnnouncementsScreen (`GET /v1/notice` page/size 페이징), AnnouncementDetailScreen (`GET /v1/notice/{id}`). 카드: 제목·본문 1줄 말줄임, publishedAt YYYY-MM-DD 우하단. 빈 상태 l10n.noticesEmpty. showYn='n' 또는 삭제/404 시: 상세 페이지 대신 `DefaultPopup`으로 l10n.postNoLongerAvailable(게시물이 삭제되었거나 존재하지 않습니다) 안내, 버튼 l10n.backToHome(홈으로 가기).
- **Tutorial 스크린 추가**: Overview → Opening 전환 전 TUTORIAL metaInfo 조건 분기
  - `lib/screens/roleplay/tutorial.dart` (RoleplayTutorialScreen): 6페이지 슬라이드, 상단 dot indicator, 마지막 페이지 탭으로 완료
  - 조건: `userDto.metaInfo`의 `TUTORIAL != 'Y'`인 경우 노출, 이미 완료 시 즉시 Opening으로 skip
  - 언어별 이미지: `assets/images/tutorials2/{ko|pt|en}/tutorial-{1~6}.png`
