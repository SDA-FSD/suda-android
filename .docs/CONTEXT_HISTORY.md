...
- **Result V2 1차 박스레이어 구현**: `lib/screens/roleplay/result_v2.dart`를 중앙 박스레이어 중심 구조로 재구성. 전체 배경은 상단 `#054544` → 하단 `#0CABA8` 그라데이션 유지. 박스레이어는 별 행 위에 레거시 `result.dart`와 동일한 `SizedBox(height: 20)` 여백 후 별 3개 + mainTitle + 민트색 subTitle + Mission/Words/Like 3카드로 구성하며, Like 카드는 `assets/images/like_at_result.png` 24×24와 기존 Result와 동일한 그라데이션 숫자를 사용한다.
- **Result V2 진입 후 타이밍**: bottom-up 라우트로 화면이 fully shown 된 시점을 `ModalRoute.animation` 완료로 감지하고, 1초 뒤 박스레이어를 상단 영역으로 이동시키면서 동시에 `LikeProgressEffect.play()`를 실행한다. effect 파라미터는 `beforeLikePoint`, `afterLikePoint`, `beforeLevel`, `afterLevel`, `beforeProgressPercentage`, `afterProgressPercentage`를 사용한다. 현재 단계에서는 effect 완료 후 임시로 `done`만 표시하고, 본문레이어는 아직 구현하지 않는다.
