...
- **카드 프레임 (glassy)**: radius `16`, `BackdropFilter` blur sigma `12`, 테두리 흰색 36%·1px, frost 그라데이션 흰색 18%→10% (topLeft→bottomRight), 그림자 검정 14%·blur 10·offset (0, 2). **전체 dim** 검정 40% (`0x66000000`). `barrierDismissible`일 때 dim 탭으로 닫기.
...
- **primary 버튼 폭**: 기본은 라벨 shrink-wrap. `expandPrimaryButtons: true`면 primary·primaryLight만 카드 콘텐츠 가로 전체(`width: infinity`). Top 3 Rewards 등 호출부 선택.
- **버튼 타입**: `primary`(틸 fill `#0CABA8`·흰 글자), `primaryLight`(흰 fill·검정 글자, Unfriend 등), `text`(TextButtonTheme)
