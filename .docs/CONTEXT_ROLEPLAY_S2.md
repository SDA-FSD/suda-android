...
| 단계 | 내용 | 데이터/API | 코드 상태 |
|------|------|------------|-----------|
| **Opening** | 세션 생성 | `POST /rps2/sessions` → `RpS2SessionDto` | ✅ `opening.dart`. AOS Start는 briefing `stop()`을 2s timeout 후 전환(hang 시 Playing 진입). iOS는 기존처럼 `stop()` await. |
