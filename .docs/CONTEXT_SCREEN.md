...
- **스크린 타입 특성**: Main Screen
  ...
  - GNB 색상: 검정 (`Colors.black`) - 시스템 네비게이션 바와 색상 통일
  - 본문 `SingleChildScrollView` bottom inset = `GnbBar.contentHeight`(Ranking과 동일). 배경은 GNB 뒤까지, 마지막 콘텐츠는 GNB 위에. `MediaQuery.padding.bottom`은 SafeArea와 중복하지 않음
- **UI 구성**: `AppScaffold`를 사용하여 표준 레이아웃 적용
  - **상단 여백**: 70 (표준)
...
- 상단 4:6. 우측 블록 start 마진 20. 좌 `rpImgPaths[0]` 등급 링 두께 5·CDN 원본·컬렉션과 같은 우하단 섀도. 우 이름 `headlineMedium` 흰 + `[Normal|Rare|Epic]` `bodySmall`. 한 줄 갭 후 Age/Nationality/Occupation/Interests. 라벨 Bold, 값 없으면 행 숨김. Personality는 `personalities`만 알약(`#635F5F`, labelSmall italic 흰, gap 10)
- Collection: 최소 3칸, `rpImgPaths`가 더 길면 그 수만큼 가로 스크롤. 원마다 우하단 소프트 섀도(`#000` 40%, offset 원폭 4%/8%, blur 16%). 갭 24. 소유 칸 `_150`. 현재 프로필이면 우하단 `#0CABA8` 30 체크. 소유·비설정이면 아래 `characterSetAsProfile` → 확인 팝업(미리보기 원형은 등급 테두리 없음. 컬렉션·확대 공통) 후 `PUT /v1/users/profile-img` `{type, value}`. 저장값만 `TYPE:value`. `GET /v1/users`의 imgPath가 그 값과 같으면 체크·문구를 옮기고, 아니면 실패 토스트. 실패 토스트 `characterChangeFailed`. 미소유·빈 칸은 `#570B3C` 64% + 등급색 `lock.png`(원 폭 40%). 탭은 소유 칸만. 확대 오버레이의 Set as Profile은 현재 프로필이 아닐 때만. 이미지와 간격 48. `ElevatedButton`(`elevatedButtonTheme`) 배경 `#0CABA8`·글자 흰색
...
- **ChangeProfileImageScreen** (Sub Screen): 아바타 또는 `accountChangePicture` 탭 시
...
- 푸터는 서비스 메시지 24px, 녹음/타이핑 입력, 하단 mic/keyboard·에너지·hint 아이콘 3층 구조. 녹음 영역 높이 140(`roleplayMicFooterStackHeight`). 중앙 `PlayingEnergyIndicator`(일반: energy+숫자, 무제한: unlimited 아이콘만). 30분 충전 00:00 시 서버 재조회. 발화 성공 시 로컬 -1. 에너지 0에서 **녹음 또는 타이핑 send** 또는 user-message `402` 시 `showPlayingEnergyInsufficientPopup`(버튼 `endRoleplay`) → Wait 레이어 후 Overview 이탈. **세션당 첫 사용자 발화 턴**에만 `holdMicrophoneToSpeak` fade-in/out, 마지막 턴 나레이션·후속 AI 종료 후 서버 `serviceMessage`(없으면 `roleplayAnalyzing`) blink. 오토힌트 ON으로 힌트박스가 자동 노출된 턴은 사용자 턴 활성 후에도 힌트 버튼 disabled를 유지한다. 녹음 시작 완료 전 release/cancel이 들어오면 pending action으로 보관해 start 완료 직후 finish/cancel을 이어서 처리한다. **녹음 가드**(`playing_input_mixin`): begin/cancel/finish/stop 직렬화·stop/cancel 2s timeout·try-catch·이탈(`_confirmExit`)/dispose/`lockPlayingInputForSessionEnd` 전 `teardownPlayingRecording()`(cancel 완료 대기) — `record` MediaMuxer race 크래시 완화. **iOS 초반 잘림 완화**: AI 음성 종료 시·`activateUserTurn`에서 `playAndRecord` 세션 prewarm, 권한 캐시, `start()` 성공 후에만 `_recordingStartedAt` 기록, prewarm/탭 동시성은 in-flight Future로 직렬화.
- API: 사용자 음성 `POST /rps2/sessions/{id}/user-message/audio`(octet-stream), 텍스트 `POST /rps2/sessions/{id}/user-message/text`(raw string), 후속 AI 음성 `GET /rps2/sessions/{id}/ai-message/audio`. 상황 로그 `POST /rps2/sessions/{id}/event-logs`(실패 무시, 상세 `CONTEXT_ROLEPLAY_S2.md`).
- 사용자 발화 응답 후 타이밍: 사용자 말풍선 노출 직후 후속 AI 음성을 미리 준비하고, 500ms 후 나레이션을 `TextPainter` 시각 줄마다 fade-in(최소 1초 단계 보장, 공백 없는 ja/zh/th 포함)한 뒤 500ms 대기한다. 이 시점과 AI 음성 준비 완료 중 늦은 시점에 AI 말풍선을 노출하고 준비된 음성을 재생한다.
...
- `RpS2UserHistoryDto` 전달. USER 카드 펼침/TTS는 Result와 동일 (`feedbackLockedYn`). 상세 `CONTEXT_ROLEPLAY_S2.md` Speech Feedback. iOS TTS는 Result와 동일 `SudaTtsAudioPlayer`.
...
## 25. OtherUserProfileScreen

### 스크린 관련 정의 파일
- **파일 경로**: `lib/screens/other_user_profile.dart`
- **클래스명**: `OtherUserProfileScreen` (StatefulWidget)
- **스크린 타입**: **Sub Screen** (좌상단 백버튼. GNB 없음)
- **appPath**: `/profile/{userId}` (숫자 id. `/profile/history`·`/profile/setting`과 구분). 본인 id면 GNB Profile만 열고 SubScreen은 열지 않음

### 스크린 용도
- Ranking COLLECT/ANNOUNCE에서 **다른 사용자**의 읽기전용 프로필. 친구 알약만 쓰기.

### 이전 스크린 정보 (진입점)
- **RankingScreen**: 포디움·4위+ 리스트의 **이미지·이름**. 빈 칸·본인(`isMe`/`user.id`)·Like 숫자·Ranking Reward Claim 패널 제외
- 푸시 `appPath` `/profile/{userId}`

### 이후 스크린 정보
- 백 → 이전 스크린. 설정·Paywall·Saved/History 없음
...
- Neighbors: 타이틀 항상. 획득 있으면 내 프로필과 같은 가로 스크롤(탭 없음, 등급 링 두께 3). 0개면 `otherUserNeighborsEmpty`(en No characters collected yet. / pt Nenhum personagem coletado ainda. / ko 아직 획득한 캐릭터가 없습니다.)
