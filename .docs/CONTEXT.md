...
- `UserDto`: provider/sub/name/email/`imgPath`, 통계, `metaInfo`(`SudaJson`). `upsertMetaInfo` / `hasMetaInfoValue`. `imgPath` null/empty는 1번 노랑 `#FFB700` (`UserImgPath.fallbackColor`)
- Main 복귀 시 `_syncUserOnMainRouteReturn` → `GET /v1/users`. Profile 헤더는 `GET /v1/users/my-profile` (id/name/imgPath/currentLevel/likePoint/friendCount). Progress 탭은 `GET /v1/users/progress` (streak/words/`progressPercentage`/currentLevel/`claimableRewardLevel`/`claimableCharacterRewardId`/`claimedCharacters`/`achievements`) — 탭만 재조회 가능. 기존 `/v1/users/profile` 클라 폐기. 수령 초상화·업적은 Progress. LEVEL_UP 선물은 `POST /v1/users/character-rewards/claim`. 다른 사용자 프로필은 `GET /v1/users/{userId}/profile`(헤더+progress+`subscribedYn`+`claimedCharacters`+`relationStatus`). 친구 목록 `GET /v1/users/friends`. 쓰기는 `POST /v1/users/friends/{targetUserId}` · `POST …/accept` · `POST …/reject` · `DELETE …/request` · `DELETE …/{targetUserId}`. 내 프로필 Friends 칸만 `FriendsScreen`(타인 친구 목록 없음). Character Full Screen은 `GET /v1/characters/{id}` (`CONTEXT_SCREEN.md`). 본인 id의 appPath `/profile/{userId}`는 GNB Profile.
...
- **CDN 썸네일** (`lib/utils/cdn_thumbnail.dart`, `lib/widgets/cdn_thumb_image.dart`): 원본 path `…/zj66s0.png` → `{stem}_{size}.{ext}` (`_300` 현행, `_150` Profile 아바타 등, `_500` enum 예약) 후 `cdnBaseUrl` prepend. 파생 파일이 없으면 원본. 슬롯 `CdnThumbSlot`→`CdnThumbSize`.
  - **작은 노출** (`CdnThumbImage`): Home 가로 행·Similar Topic 3열(`HomeSeriesDto.thumbnailImgPath`)·Episode 탭 좌측 카드(`RpS2SeriesEpisodeDto.thumbnailImgPath`)·Profile 히스토리 그리드(`RpS2SimpleHistoryDto.imgPath`, `_300`)·Profile 아바타(`MyProfileDto.imgPath`, `CdnThumbSlot.profileAvatar` `_150`)·GNB 아바타(`UserDto.imgPath`, `gnbAvatar` `_150`)·랭킹 아바타(`RankEntryDto.imgPath` CDN path, `rankAvatar` `_150`)·Change Profile Image 그리드(`characterImgPath`, `characterReward` `_150`). **Change Profile Image 상단 미리보기**와 **Reward Unboxing** 캐릭터(`characterImgPath`)는 `_150` 없이 CDN **원본**만 (`CdnThumbUrl.original`).
  - **프로필 링** (`UserProfileAvatar` / `UserProfileRing`): 링은 하나만. 구독이면 구독 그라데이션(`#80D7CF→#8A38F5`). 비구독 캐릭터면 같은 두께의 등급 그라데이션. 그 외 무료(`#80D7CF→#43716D`). 두께는 외경의 4%(100 → 4). 포디움 1위 금링·2·3위 비구독 포디움 링·Ranking Reward Claim 전용 링은 유지하고, 그 **안쪽**만 구독 링 / 등급 링(두께 5) / 없음. GNB는 흰 테두리만. Neighbors는 등급 링만(두께 3, 구독 무관). Unboxing은 등급 링만(두께 10, 구독 무관). 최초 프로필 캐러셀은 테두리 없음.
...
- 동의(`SUDA_AGREEMENT`): Login 위 레이어. 완료 시 `POST /v1/users/agreement` + AppsFlyer `af_complete_registration` → **1회** `FirstCefrLevelScreen` → Confirm `PUT /v1/users/language-level`(실패여도 다음) → **1회** `FirstProfileImageScreen` → Confirm `PUT /v1/users/profile-img` `{type:DEFAULT,value:"1"…"4"}` 성공 시 `GET /v1/users`로 `_user` 갱신 후 Home(실패여도 Home). 레이어 닫기(미동의)는 토큰 삭제·비로그인
- Lab: Setting > Lab (`AppConfig.isDev` · `kDebugMode`). **prd release 미노출**. Tutorial 미리보기: 21 locale 선택 후 Open Tutorial (`RoleplayTutorialScreen(preview: true)`). **Open First CEFR Level** / **Open First Profile Image**. **Open Ranking Announce Preview**: `Ranking(forceAnnouncePhase: true)`로 ANNOUNCE UI만 재현(실데이터 로드). **Open Ranking Reward Claim Preview (1st|2nd|3rd)**: `Ranking(forceRankingRewardClaimPreview: true, forceRankingRewardClaimPlace: 1|2|3)` — GNB 위 Ranking Reward Claim 패널. **Open Reward Unboxing**: `CharacterRewardClaimDto.labUnboxingPreviewItems()` 3건 + `RewardUnboxing.preload`/`push`. **Play Ribbon Burst**: `RibbonBurstEffect` 오버레이 재현
- Ranking 카운트다운 (`ranking.dart`/`Ranking`): ≥48h 일 / 24h~48h 1일 / 1h~24h 시 / 1h 미만 분. l10n `rankCountdownDays|Hours|Minutes`(en: `1 day`·`N days left`·`N min`). 시계+문구 색 `#FFFFFF` ~70% 알파
- Ranking `?` Top 3 Rewards (`top_3_rewards_popup.dart`/`DefaultPopup`): title↔desc 사이 `#D9D9D9` 20% 1px 선. 1~3위 카드 glassy(white fill~10%·border~24%·radius 20, 중첩 blur 없음). place `headlineSmall` 흰 Bold20 중앙. 보상 행 아이콘(`assets/images/achievement/medal_1st|2st|3st`·`like_at_result`·`assets/images/achievement/reward_box`)+`bodyMedium` 흰 Italic16. Okay `expandPrimaryButtons`
- Ranking Reward Claim (`ranking_reward_claim.dart`/`RankingRewardClaimPanel`): 1~3등 미수령 UI. Dialog 아님 — 랭킹 Scaffold 위 **풀스크린 레이어**(GNB 유지). **배경·sunburst는 GNB 뒤까지**(글래시), 콘텐츠(Claim 버튼)는 GNB+safe inset. 등장은 **DefaultPopup과 동일 계열**(dim `#000@40%` + 페이드 + 중앙 스케일 0.9→1, ~150ms). **자동 노출:** 서버 `period.phase==ANNOUNCE`이고 `/entries/me` 1~3위이면 `GET /v1/rank/character-rewards/claimable?periodId=` → 200이고 id 1개 이상이면 해당 등수로 패널. 실패·`[]`·periodId없음·Lab `forceAnnouncePhase`만(실phase 아님)은 미노출. Lab `forceRankingRewardClaimPreview`+`forceRankingRewardClaimPlace`는 이 게이트와 무관(Claim 시 id 없으면 Lab 목데이터로 Unboxing). Claim 버튼: `POST /v1/rank/character-rewards/claim` body=`List<Long>`(claimable ids). POST·Unboxing CDN 원본 프리로드 동안 버튼 스피너·동기 플래그로 따닥 무시. 200 + 1개 이상이면 `FullScreenTransition.fade`로 `RewardUnboxing`을 **먼저 덮은 뒤** fade 완료 후 Ranking Reward Claim 레이어 제거(랭킹 본문 플래시 방지). 200 `[]`는 레이어만 닫음. 실패는 `rankingRewardClaimError` 토스트·레이어 유지. like는 응답에 없음(등수당 1회 서버 지급). 1등 배경 `#FF00A6`→35%`#8A38F5`→`#80D7CF` / 2등 `#FF00A6`→`#8A38F5` / 3등 `#0CABA8` 단색. sunburst `ProceduralSunburstOverlay`(`SunburstFocal.bottomCenter`, 회전 pivot도 `Alignment.bottomCenter`·Unboxing과 동일 wedge·48s/64s·opacity 0.55). 2·3등 보더 흰→`#0D7F7D`(`rankingRewardClaimRunnerUp`). 워터마크 서수 숫자80+접미사50. Rewards 카드 폭 0.75, 3등 fill만 `BlendMode.overlay`. Like pill `#043B3A@90%`. `avatarScale=s*1.85`. l10n `rankingRewardClaim*`+`rankTop3*`
- Reward Unboxing (`lib/screens/reward/reward_unboxing.dart`/`RewardUnboxing`): 캐릭터 보상 언박싱 **Full Screen**(GNB 없음). fade 200ms. `List<CharacterRewardClaimDto>` n건을 차례로 3단계(선정→탭→확정→해금). 진입 전 `RewardUnboxing.preload` CDN 원본. DTO에 `characterName`. 원형 테두리 공용 `CharacterRarityFrame`(NORMAL `#71A431→#2B3E13` / RARE `#009DFF→#0CABA8` / EPIC `#DF3FF8→#51218F`, border 10). **선정:** 배경 `#FF00A6→#8A38F5→#FF00A6`, `ProceduralSunburstOverlay`(`SunburstFocal.center`; 흰 10%/20% wedge·opacity 0.55·48s/64s), `reward_box.png` 폭 55% 주기 떨림+약진동, 화면 75% 높이 `rewardUnboxingTapToOpen`. **탭 후 1.5s:** 검정 마스크·안내 숨김·진동 가속 → rarity 닫힌 박스 fade(`reward_box_{normal|rare|epic}.png`) + 배경(NORMAL `#80D7CF` / RARE `#9A30EA@80%→#80D7CF→#8A38F5` / EPIC 최상·최하 `#FF00A6@20%`, 25% `#9A30EA@87%`, 중앙 `#80D7CF`, 75% `#8A38F5`) + 강진동. **확정+1s:** opened 박스(에셋 비율×`_openedDisplayScale` 1.10, 닫힌 슬롯 중앙 정렬)+강진동 2회+캐릭터 CDN 원본이 박스에서 폭 50%로 확대·최종 중심 Y=(legacy `height×0.25` + opened-뚜껑 기준 Y)/2(opened를 가림). **확정+500ms:** 상단 카피(캐릭터 상단~safe 구간 하단 정렬·gap12; `newCharacterYn==Y`만 `rewardUnboxingNewCharacter` / `rewardUnboxingYouGot({characterName})` / `[Normal|Rare|Epic]` 영어, 그림자 `Offset(0,4)` blur4 `#000@25%`)·진행 `{current} / {total≤3이면 3}`(`headlineMedium`, opened 하단부터·Column 이중 Spacer)·비마지막 `Open Next Box`(Claim과 동일 흰 필, 영어만)·마지막 `Set as Profile`+아래 `View Character`(동일 흰 필). current==보정total이면 `secret_unlocked.png`(폭 40%, 캐릭터·opened 사이·초기(우단=중앙선)와 중앙 보정의 중간, reveal 완료 후 `_secretController` 400ms fade). Set as Profile: `PUT /v1/users/profile-img` `{type:NORMAL|RARE|EPIC,value:characterImgPath}` 후 `GET /v1/users`로 `_user` 갱신(`MainUserSync`)·프로필 탭 전환·Unboxing pop. API 실패여도 이동. View Character 무동작. 마지막 해금 완료 후 좌상단 `close.svg`. 시스템 백도 그때만. Profile Progress 레벨업 선물은 `POST /v1/users/character-rewards/claim` 후 진입.
...
- Ranking 포디움 1~3위: Figma 440 기준 가로 스케일. **4~10 리스트(7행)가 한 화면에 들어가게** 세로가 부족하면 포디움 전체(프로필·왕관·레벨 뱃지 비율 유지)를 축소. 1위 `ranking_1st_crown`+흰→금 링(유지). 2·3위 구독은 구독 링만, 비구독은 포디움 링(흰→`#0CABA8`) 유지. 전용 링 안쪽은 구독 링 / 등급 링 / 없음. 리스트 4위+는 프로필과 같은 단일 링. 이름 Bold 14·좋아요 Regular 14·아이콘 14×14. 3위가 2위보다 살짝 아래(Y 212>204). 레벨 뱃지 `#0CABA8` 30
...
## 11. 코드 습관
- 폰트: 가변 `ChironHeiHK` / `ChironGoRoundTC`. APK는 `latn` 팩만. 나머지 locale 팩은 CDN → documents `fonts/v1/` (`FontPackService`). AOS TTF `noCompress` 유지. 스플래시 해제는 JWT. 상세 `CONTEXT_STYLE.md`.
...
- 리본 폭죽 오버레이: `RibbonBurstEffect` (`lib/effects/ribbon_burst_effect.dart`) — 화면 중앙 3D 직사각형 리본 burst. 딤·입력 가로채기 없음. Ranking Reward Claim 보조 연출(예정). Lab `Play Ribbon Burst`
