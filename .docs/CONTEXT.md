...
- Main 복귀 시 `_syncUserOnMainRouteReturn` → `GET /v1/users`. Profile 헤더는 `GET /v1/users/my-profile` (id/name/imgPath/currentLevel/likePoint/friendCount). Progress 탭은 `GET /v1/users/progress` (streak/words/`progressPercentage`/currentLevel/`claimableRewardLevel`/`claimableCharacterRewardId`/`claimedCharacters`/`achievements`) — 탭만 재조회 가능. 기존 `/v1/users/profile` 클라 폐기. 수령 초상화·업적은 Progress. LEVEL_UP 선물은 `POST /v1/users/character-rewards/claim`. 다른 사용자 프로필은 `GET /v1/users/{userId}/profile`(헤더+progress+`subscribedYn`+`claimedCharacters`+`relationStatus`). 친구 쓰기는 `POST /v1/users/friends/{targetUserId}` · `DELETE …/request` · `DELETE …/{targetUserId}`. 본인 id의 appPath `/profile/{userId}`는 GNB Profile.
