...
- **CDN 썸네일** (`lib/utils/cdn_thumbnail.dart`, `lib/widgets/cdn_thumb_image.dart`): 원본 path `…/zj66s0.png` → `{stem}_{size}.{ext}` (`_300` 현행, `_150` Profile 아바타 등, `_500` enum 예약) 후 `cdnBaseUrl` prepend. 파생 파일이 없으면 원본. 슬롯 `CdnThumbSlot`→`CdnThumbSize`.
  - **작은 노출** (`CdnThumbImage`): Home 가로 행·Similar Topic 3열(`HomeSeriesDto.thumbnailImgPath`)·Episode 탭 좌측 카드(`RpS2SeriesEpisodeDto.thumbnailImgPath`)·Profile 히스토리 그리드(`RpS2SimpleHistoryDto.imgPath`, `_300`)·Profile 아바타(`MyProfileDto.imgPath`, `CdnThumbSlot.profileAvatar` `_150`)·GNB 아바타(`UserDto.imgPath`, `gnbAvatar` `_150`)·랭킹 아바타(`RankEntryDto.imgPath` CDN path, `rankAvatar` `_150`). **Reward Unboxing** 캐릭터(`characterImgPath`)는 `_150` 없이 CDN **원본**만 (`CdnThumbUrl.original`).
