...
- 응답 `{ successYn, pendingYn, finishYn }`. AOS는 `finishYn` 무시(consume/ack 서버). **iOS는 `finishYn=Y`만** `completePurchase`. Play `PurchaseStatus.pending` 또는 verify `pendingYn=Y`는 **실패가 아님**: 스피너 종료 + `energyPurchasePendingApproval` 토스트 + **현재 화면 유지**(에너지 슬롯 숨김·Paywall Completed/`pop(true)` 없음). 이후 purchased는 orphan verify. 지급은 웹훅+다음 energy refresh.
- Restore/Change Plan verify: `offerSessionId`/`paywallSessionId` 없음. Restore는 트랜잭션당 1회, **소모품 호출 금지**. 미finish 재시도: iOS는 다음 `purchaseStream`(StoreKit 재전달) + 같은 verify. **AOS는 `purchaseStream` + `queryPastPurchases`**(Play는 미ack을 스트림으로 자동 재전달하지 않음). Home `ensureListening`·buy 미런치(`ITEM_ALREADY_OWNED` 등)에서도 query 회수. `purchaseToken`당 verify 1회.
...
- 에너지 팝업 구매 성공: 해당 슬롯 1초 숨김 애니 후 `GET energy/detail` refetch. **AOS resume grace 최대 10s**(Play 시트 dismiss). 매칭 스트림이 오면 즉시 종료(purchased→verify, pending→승인대기 토스트). 만료 시 `queryPastPurchases`로 purchased/pending 회수, 없으면 `storeDismissed`. iOS는 스트림의 purchased/canceled/pending 대기(grace 없음). AOS도 미ack/미consume orphan verify. iOS orphan/simple bus가 와도 팝업은 detail을 다시 받아 같은 애니로 슬롯을 가림. Paywall/팝업 dispose `abandonPendingPurchase`는 UI Future만 닫고, 이후 콜백은 세션을 이어받아 orphan verify.
