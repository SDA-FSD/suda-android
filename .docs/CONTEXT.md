...
- **API 서버 연동**: `lib/api/suda_api_client.dart`
  ...
  - `SudaApiClient.getUserEnergy()`: 에너지 **상세(팝업용)** 조회 (`GET /v1/users/energy/detail?screen={home|opening|opening_insufficient|playing|playing_insufficient}`, 응답: 위 코어 + **showUnlimitedPurchaseYn**, **showCapacity6PurchaseYn**, **showCapacity7PurchaseYn**, **offerSessionId**). **에너지 팝업 오픈·팝업 내 재조회·구매 후 상품 영역 갱신에만 사용**(서버가 구매이력 조회해 플래그 계산).
    ...
    - **에너지 팝업 상품 탭 impression**: `EnergyPurchaseSection`에서 INAPP 버튼 탭 시 `POST /v1/users/impression/product` 호출(응답 무시). `offerSessionId`는 detail 응답의 최신 값 사용, `productId`는 해당 INAPP productId. 동일 `(offerSessionId, productId)`는 팝업 생명주기 내 1회만 전송(클라 따닥 방지). Go Premium/ Lab은 수집 대상 제외.
...
- **Change Plan (실측 중)**: `IapPurchaseService.changeSubscription` — `queryPastPurchases` + `ReplacementMode.withoutProration`(같은 구독 base plan 전환; DEFERRED는 Play가 거절해 교체). old purchase 없으면 `oldPurchaseNotFound`. verify는 기존 경로 — **실측 시 서버 로그 병행**.
