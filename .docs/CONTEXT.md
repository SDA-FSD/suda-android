...
- Ranking Reward Claim (`ranking_reward_claim.dart`/`RankingRewardClaimPanel`): 1~3등 미수령 UI. Dialog 아님 — 랭킹 Scaffold 위 **풀스크린 레이어**(GNB 유지). 등장은 **DefaultPopup과 동일 계열**(dim `#000@40%` + 페이드 + 중앙 스케일 0.9→1, ~150ms). Lab `forceRankingRewardClaimPreview`+`forceRankingRewardClaimPlace`. 1등 배경 `#FF00A6`→35%`#8A38F5`→`#80D7CF` / 2등 `#FF00A6`→`#8A38F5` / 3등 `#0CABA8` 단색. sunburst `sunburst_pattern_rewards.png`(1~3등, 440×956 비율, 하단 정렬, PNG 알파 × opacity 0.55). 2·3등 보더 흰→`#0D7F7D`(`rankingRewardClaimRunnerUp`). 워터마크 서수 숫자80+접미사50. Rewards 카드 폭 0.75, 3등 fill만 `BlendMode.overlay`. Like pill `#043B3A@90%`. `avatarScale=s*1.85`. 실 API 후속. l10n `rankingRewardClaim*`+`rankTop3*`
