...
- 에너지 팝업 구매 성공: 해당 슬롯 1초 숨김 애니 후 `GET energy/detail` refetch. **AOS resume grace 최대 10s**(Play 시트 dismiss). 매칭 스트림이 오면 즉시 verify·스피너 종료. 만료 시에만 `queryPastPurchases`로 회수하고, 없으면 `storeDismissed`. iOS는 스트림의 purchased/canceled 대기(grace 없음). AOS도 미ack/미consume orphan verify. iOS orphan/simple bus가 와도 팝업은 detail을 다시 받아 같은 애니로 슬롯을 가림. Paywall/팝업 dispose `abandonPendingPurchase`는 UI Future만 닫고, 이후 콜백은 세션을 이어받아 orphan verify.
