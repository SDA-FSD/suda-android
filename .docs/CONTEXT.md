...
- **API 서버 연동**: `lib/api/suda_api_client.dart`
  ...
  - `SudaApiClient.getUserEnergy()`: 에너지 **상세(팝업용)** 조회 (`GET /v1/users/energy/detail`, 파라메터 없음, 응답: 위 코어 + **showUnlimitedPurchaseYn**, **showCapacity6PurchaseYn**, **showCapacity7PurchaseYn**). **에너지 팝업 오픈·팝업 내 재조회·구매 후 상품 영역 갱신에만 사용**(서버가 구매이력 조회해 플래그 계산).
    ...
    - **구독 만료 재조회**: `subscriptionExpiredAt` 경과 시 1회 `GET /v1/users/energy/simple` 재조회 (`EnergyTimerRefetchTracker`). 충전(`lastAutoChargedAt`)처럼 1초마다 API를 치지는 않음.
