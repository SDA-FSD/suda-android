...
- Ranking ANNOUNCE phase: 결과 화면 전용 구성 — 타이틀(`rankAnnounceTitle`, Outer Shadow `Offset(0,4)` blur4 `#000000` 25%) + 1~3 포디움 + 배경 장식(`sunburst_pattern.png` 440좌표 left=-255,top=-3,949×949, PNG 알파~14% + `BlendMode.softLight` — 레이어 opacity 이중 적용 금지 → `confetti.png` left=-15,top=12,453×185). 타이틀·포디움 블록 Y+64(내부 간격·크기 유지). 도움말·카운트다운·4위+리스트·sticky·미참여 오버레이는 ANNOUNCE에 없음(COLLECT 전용). Lab `forceAnnouncePhase`는 동일 ANNOUNCE UI 경로. s=contentWidth/440
